...
class MeliaeTests(unittest.TestCase):
    ...
    def test_group_children(self):
        ...
        # module, dict, moo, collection-of-strings
        assert len(index) == 4, index
    ...
    def test_recursive_shared(self):
        """Do we account for recursive structures shared across modules
        properly?

        Loops should become a single "thing" which has references and
        parents, but also can be broken down to see what is in the
        loop...

        Loop totsize is the totsize of all components of the loop
        divided by the number of references to the loop which are *not*
        ...
        records = [
            ...
            {'size': 1, 'type': 'dict', 'address': 5, 'refs': [4, 1]},
